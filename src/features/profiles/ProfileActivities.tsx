...
import { ProfileActivitiesFilter } from "@/libs/hooks/useProfileActivitiesFilters";

export default function ProfileActivities() {
  const [activeTab, setActiveTab] = useState(0);
  const { id } = useParams();
  const { userActivities, setFilter, isLoadingUserActivities } = useProfile(id);
  ...
  const handleTabChange = (_: SyntheticEvent, newValue: number) => {
    ...
    setFilter(tabs[newValue].key as ProfileActivitiesFilter);
  };
